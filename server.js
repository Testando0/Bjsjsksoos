...
const db = new sqlite3.Database('./red_protocol_v18.db');
...
app.get('/messages/:u1/:u2', (req, res) => db.all("SELECT *, strftime('%H:%M', time) as f_time FROM messages WHERE (s=? AND r=?) OR (s=? AND r=?) ORDER BY time ASC", [req.params.u1, req.params.u2, req.params.u2, req.params.u1], (e, r) => res.json(r)));
